...
export function ContributorToolbar() {
  ...
  const published = updatedAt
    ? new Date(updatedAt).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })
    : '';
  ...
  return (
    ...
    >
      <div className="flex items-center min-w-0">
        ...
        <button type="button" className={item}>Promouvoir</button>
        <button type="button" className={item}>Traduction</button>
        <button type="button" className={`${item} hidden md:inline-flex`}>Détails de la page</button>
        <button type="button" className={`${item} hidden lg:inline-flex`}>Analyse</button>
      </div>
      <div className="flex items-center shrink-0">
        {published && <span className="text-[#605e5c] px-sm hidden sm:inline">Publié le {published}</span>}
        <button type="button" className={item}>Partager</button>
