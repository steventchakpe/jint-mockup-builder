...
9. **SVG icons must come from Figma** — Never approximate or hand-write icon paths. Always export the exact SVG from the Figma node:
   ```js
   const icon = await figma.getNodeByIdAsync('NODE_ID');
   const svg = await icon.exportAsync({ format: 'SVG' });
   const decode = (arr) => { let s=''; for(let i=0;i<arr.length;i++) s+=String.fromCharCode(arr[i]); return s; };
   return decode(svg);
   ```
